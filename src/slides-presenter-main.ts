...
function syncVoiceBadge(): void {
  ...
  if (playbackState === 'idle') {
    ...
    voiceBadgeEl.innerHTML = cached
      ? `<span class="voice-badge-dot"></span>Audio ready · ${voice}`
      : `<span class="voice-badge-dot"></span>Not generated`;
    return;
  }
...
presentSlidesBtn?.addEventListener('click', () => {
  ...
  if (stageCardEl) {
    void stageCardEl.requestFullscreen().catch((error: unknown) => {
